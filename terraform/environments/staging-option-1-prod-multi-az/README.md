# Staging Option 1 + Production + Shared Multi-AZ
...
This configuration provides a cost-effective staging environment using Lightsail while maintaining a production-grade EKS platform with high-availability Multi-AZ shared infrastructure for database and caching services.
...
| EC2 EBS | Block storage | 2× 100 GB gp3 for EKS nodes | Multi-AZ |
...
- **Recovery**: Automatic for RDS/Redis, manual for EKS nodes
...
**Multi-AZ Premium**
- **RDS**: ~2x Single-AZ cost (standby instance)
- **Redis**: ~2x Single-AZ cost (replica instance)
- **Total**: Additional ~$180/month for shared infrastructure
...
cd terraform/environments/staging-option-1-prod-multi-az
...
3. **Configure backend.tf**
   ...
   terraform {
     backend "s3" {
       ...
       key            = "staging-option-1-prod-multi-az/terraform.tfstate"
       region         = "us-east-1"
       encrypt        = true
...
- State file path: `staging-option-1-prod-multi-az/terraform.tfstate`
...
**Environment Total: ~$777.05/month + usage-based charges**
...
6. Monitor Multi-AZ failover events to justify cost
...
11. **Multi-AZ**: High availability for critical data services
...
### Multi-AZ Specific Monitoring
- Monitor RDS failover events
- Monitor Redis failover events
- Track replication lag for Redis
- Monitor standby instance health
- Review cross-AZ data transfer costs
- Set up alerts for availability zone issues
