# Staging Option 2 + Production + Shared Single-AZ
...
This configuration provides a flexible staging environment using EC2 with full VPC networking while maintaining a production-grade EKS platform with single-AZ shared infrastructure for database and caching services.
...
| Amazon EKS | Production compute/orchestration | 1 cluster, 2× t3.xlarge nodes (4 vCPU, 16 GB RAM) | Single-AZ nodes |
| ALB | Application ingress | Standard ALB, HTTP/HTTPS termination | Single-AZ |
...
| EC2 EBS | Block storage | 30 GB gp3 for EC2, 2× 100 GB gp3 for EKS nodes | Single-AZ |
...
- **AZ Failure**: Complete service outage for RDS, Redis, EC2, and EKS
- **No Automatic Recovery**: Manual intervention required
- **EKS Impact**: EKS nodes in same AZ also affected
- **EC2 Impact**: EC2 instance in same AZ also affected
...
- **EC2 Recovery**: Can launch new instance in new AZ from AMI
...
cd terraform/environments/staging-option-2-prod-single-az
...
3. **Configure backend.tf**
   ...
   terraform {
     backend "s3" {
       ...
       key            = "staging-option-2-prod-single-az/terraform.tfstate"
       region         = "us-east-1"
       encrypt        = true
...
- State file path: `staging-option-2-prod-single-az/terraform.tfstate`
...
**Environment Total: ~$617.80/month + usage-based charges**
...
10. **Configure EC2 security** with hardening and monitoring
