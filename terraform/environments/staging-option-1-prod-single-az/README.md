# Staging Option 1 + Production + Shared Single-AZ

## Architecture Overview

This environment deploys a complete AWS infrastructure architecture combining:
- **Staging Option 1**: Amazon Lightsail instance (2 vCPU, 8 GB RAM, 160 GB SSD)
- **Production**: Amazon EKS cluster with 2 worker nodes (t3.xlarge, 4 vCPU, 16 GB RAM)
- **Shared Infrastructure Single-AZ**: RDS PostgreSQL (db.t3.medium) and Redis (cache.t4g.medium) in single availability zone
- **Supporting Services**: ALB, S3, ECR, VPC networking

This configuration provides a cost-effective staging environment using Lightsail while maintaining a production-grade EKS platform with single-AZ shared infrastructure for database and caching services.

## Complete AWS Service Inventory

| AWS Service | Purpose | Configuration | AZ Model |
|------------|---------|---------------|----------|
| Amazon Lightsail | Staging compute | 2 vCPU, 8 GB RAM, 160 GB SSD, IPv6 | Single-AZ |
...
| RDS PostgreSQL | Production database | db.t3.medium, 100 GB gp3 | Single-AZ |
| Redis/Valkey | Production cache | cache.t4g.medium | Single-AZ |
| ECR | Container registry | Private registry, ~20 GB images | Regional |
| S3 | Object storage | Standard storage, ~10 GB | Regional |
| VPC | Networking | Custom VPC with public/private/database subnets | Single-AZ |
| NAT Gateway | Private subnet egress | Single NAT Gateway | Single-AZ |
| EC2 EBS | Block storage | 2× 100 GB gp3 for EKS nodes | Single-AZ |

## Exact Instance Sizes

### Staging Lightsail
- **Instance Type**: Amazon Lightsail bundle
- **vCPU**: 2
- **RAM**: 8 GB
- **Storage**: 160 GB SSD
- **IPv6**: Enabled
...
- **Lightsail**: 160 GB SSD
- **EKS Worker Nodes**: 2 × 100 GB gp3 = 200 GB total
- **RDS PostgreSQL**: 100 GB gp3
...
- **Availability Zones**: 1 AZ (Single-AZ architecture)
- **Public Subnets**: 10.0.0.0/24
- **Private Subnets**: 10.0.1.0/24
- **Database Subnets**: 10.0.2.0/24
...
|----------------|-----------|----------|------|-------------------|--------|
| Lightsail SG | Inbound | TCP | 22 | 0.0.0.0/0 | SSH access |
| Lightsail SG | Inbound | TCP | 80 | 0.0.0.0/0 | HTTP |
| Lightsail SG | Inbound | TCP | 443 | 0.0.0.0/0 | HTTPS |
...
flowchart TD
    ...
    EKS_Workloads -->|TCP 5432| RDS_PostgreSQL
    EKS_Workloads -->|TCP 6379| Redis
    EKS_Workloads -->|HTTPS| ECR
    EKS_Workloads -->|HTTPS| S3
    EKS_Workloads -->|via NAT| Internet
    Internet -->|TCP 22| Lightsail
    Internet -->|TCP 80| Lightsail
    Internet -->|TCP 443| Lightsail
```
...
- **Protocol**: TCP
- **Source**: Internet
- **Destination**: ALB, Lightsail
- **Purpose**: Web traffic
- **Public/Private**: Public
- **Security Group**: ALB SG, Lightsail SG
...
**Port 22 (SSH)**
- **Protocol**: TCP
- **Source**: Internet (restrict in production)
- **Destination**: Lightsail
- **Purpose**: Server administration
- **Public/Private**: Public
- **Security Group**: Lightsail SG
...
- **Public Subnets**: Host ALB and Lightsail with direct internet access
...
- **Lightsail**: No IAM role (managed service)
...
## Single-AZ Architecture Details

### Selected Availability Zone
- **AZ Selection**: Dynamically selected from available AZs
- **Static Assignment**: First available AZ in the region
- **No AZ Hard-coding**: Uses AWS data sources

### Why RDS is Single-AZ
- **Cost Reduction**: No standby instance charges
- **Simplified Architecture**: Reduced complexity
- **Adequate for Development/Staging**: Acceptable availability tradeoff

### Why Redis is Single-AZ
- **Cost Reduction**: No replica instance charges
- **Simplified Architecture**: No replication complexity
- **Adequate for Development/Staging**: Acceptable availability tradeoff

### Failure Implications
- **AZ Failure**: Complete service outage for RDS and Redis
- **No Automatic Recovery**: Manual intervention required
- **EKS Impact**: EKS nodes in same AZ also affected
- **Recovery Time**: Could be hours depending on restoration process

### Backup/Recovery Behavior
- **RDS Recovery**: Can restore from snapshot to new AZ
- **Redis Recovery**: Can restore from snapshot to new AZ
- **Data Loss**: Potential data loss since last backup
- **Application Impact**: Extended downtime during recovery

### Cost Advantage
- **RDS Savings**: ~50% compared to Multi-AZ
- **Redis Savings**: ~50% compared to Multi-AZ
- **NAT Gateway**: Single NAT (reduced cost)
- **Network Traffic**: Reduced inter-AZ data transfer

### Availability Tradeoff
- **SLA**: No AWS Multi-AZ SLA benefits
- **Recovery Time**: Extended (manual intervention)
- **Data Risk**: Potential data loss
- **Suitability**: Development, testing, non-critical workloads
...
**aws_subnet.public[*]**
- **Purpose**: Public resources (ALB, Lightsail)
- **Dependencies**: VPC
...
### Staging Lightsail Resources

**aws_lightsail_instance.this**
- **Purpose**: Staging compute instance
- **Dependencies**: None
- **Security Controls**: Lightsail firewall, SSH key
- **Networking**: Public IP, IPv6
- **Storage**: 160 GB SSD
- **Encryption**: Managed by AWS
- **Availability**: Single-AZ
- **Lifecycle**: Manually deleted
- **Why Exists**: Cost-effective staging environment

**aws_lightsail_instance_public_ports.this**
- **Purpose**: Configure firewall for Lightsail
- **Dependencies**: Lightsail instance
- **Security Controls**: Port restrictions
- **Networking**: Controls inbound traffic
- **Storage**: N/A
- **Encryption**: N/A
- **Availability**: Regional
- **Lifecycle**: Manually deleted
- **Why Exists**: Network security for Lightsail
...
- **Availability**: Single-AZ (2 nodes)
- **Lifecycle**: Manually deleted
...
- **Why Exists**: Relational database for applications
...
- **Availability**: Single-AZ
- **Lifecycle**: Manual deletion requires final snapshot
- **Why Exists**: Caching layer for applications
...
3. **Configure backend.tf**
   ...
   terraform {
     backend "s3" {
       ...
       key            = "staging-option-1-prod-single-az/terraform.tfstate"
       region         = "us-east-1"
       encrypt        = true
...
- State file path: `staging-option-1-prod-single-az/terraform.tfstate`
...
**Staging Lightsail:**
- Instance: ~$39.24/month
- S3 Storage: ~$0.215/month
- **Staging Total: ~$39.46/month**
...
**Shared Infrastructure Single-AZ:**
- RDS PostgreSQL: ~$90.48/month
- Redis: ~$59.13/month
- ECR: ~$2.00/month
- **Shared Total: ~$151.61/month**

**Environment Total: ~$621.60/month + usage-based charges**
...
2. **SSH Access**: SSH access required for Lightsail (restrict in production)
...
9. **Availability**: Single-AZ design accepted for cost optimization
10. **Region**: User must specify target AWS region

## Known Limitations

1. **Single-AZ**: No automatic failover for RDS or Redis
2. **NAT Gateway**: Single point of failure, no redundancy
3. **No SSL Certificate**: HTTPS requires ACM certificate setup
4. **No Route 53**: DNS management not included
5. **No Monitoring**: Enhanced monitoring not enabled
6. **No WAF**: Web Application Firewall not included
7. **No CloudFront**: CDN not included
8. **No Lambda**: Serverless components not included
9. **No API Gateway**: API management not included
10. **No Data Pipeline**: ETL/logic not included
...
5. **IMDSv2**: Required for EKS nodes
